...
from neural_mass.event_detection import mask_segments
from neural_mass.kcomplex_features import event_iou, teager_energy
...
def select_threshold_by_cv(datasets, thresholds=None, random_state=100):
    """Select the threshold that maximises mean F1 across leave-one-out folds.

    Parameters
    ----------
    datasets : list of dicts with keys X, y, windows, filtered, sfreq, expert_events, signal
    thresholds : sequence of float or None (defaults to 0.30–0.70 in 0.05 steps)
    ...
    for test_idx in range(n):
        ...
        for t in thresholds:
            ...
            from neural_mass.event_scoring import score_events
            score = score_events(expert_events, detected)
            threshold_f1s[t].append(score["f1"])

    mean_f1 = {t: float(np.mean(vals)) for t, vals in threshold_f1s.items()}
