...
def spindle_detection(signal : NDArray,
    ...
    spindle_sequence_length = 0
    true_idxs, potential_idx = [], []
    for idx, is_spindle in enumerate(mask_out):
        if is_spindle:
            spindle_sequence_length += 1
            potential_idx.append(idx)
        elif spindle_sequence_length >= min_len:
            true_idxs += potential_idx
            potential_idx = []
            spindle_sequence_length = 0
        else:
            potential_idx = []
            spindle_sequence_length = 0
    if spindle_sequence_length >= min_len:
        true_idxs += potential_idx
    
    true_mask = np.zeros(len(signal), dtype=bool)
    for idx in true_idxs:
        true_mask[idx:idx + window_size] = True
...
def K_complex_detection(signal : NDArray,
    ...
    for i in range(len(mask_out)):
        if mask_out[i]:
            ...
            count = 0
    if count >= min_len:
        true_mask[len(mask_out)-count:len(mask_out)] = True
